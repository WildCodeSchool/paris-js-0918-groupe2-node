...
const maSuperMetaFonction = async (
  ...
  let nbrePaiementsPartiels = mesPaiementsPartiels.length + 1;
  // console.log(nbrePaiementsPartiels);

  const maMetaFonction = async (
    facture,
    mesAcomptes,
    mesAvoirs,
    dateFinCalculInterets,
    ...
    const getDateRangesWithInterestRates = (startDate, endDate) => {
      ...
      let nbreAnneesDifferences =
        fin_echeance.diff(depart_echeance, "year") + 1;
  ...
  let result = [];
  for (let i = 0; i < nbrePaiementsPartiels; i++) {
    let factureModif = facture;
    // Si il n'y a pas de paiements partiels, l'algo tourne une seule fois
    if (nbrePaiementsPartiels === 1) {
      result.push(
      ...
      // S'il y a un paiement partiel ou plus, l'algo tourne autant de fois que nécessaire
    } else {
      // Date initiale => date de premier paiement partiel
      if (i === 0) {
        result.push(
          await maMetaFonction(
            factureModif,
